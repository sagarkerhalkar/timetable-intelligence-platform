...
export default function QrTemplatesPage(){
  const [items,setItems]=useState<Template[]>([]); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [message,setMessage]=useState("");
  ...
  return <main className="qrfy-page qr-route-enter"><QrProductNav/>
    ...
    <section className="qr-template-grid">
      {items.map(item=><article key={item.id} className={`qrfy-panel qr-template-card ${item.is_default?"default":""}`}>
      ...
      {!loading&&!items.length?<div className="qrfy-panel qrfy-no-data">No saved templates yet. Create one from the QR Builder.</div>:null}
    </section>
