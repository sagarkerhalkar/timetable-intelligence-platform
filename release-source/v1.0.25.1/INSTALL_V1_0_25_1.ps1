...
$BackupRoot = Join-Path $Root "_release_backups\v1.0.25.1_$Stamp"
$LogRoot = Join-Path $Root "_release_logs"
$LogFile = Join-Path $LogRoot "v1.0.25.1_$Stamp.log"
$Applied = $false
...
function Test-QrRuntimeLifecycle {
  ...
  try {
    ...
    $templatePayload = @{
      name = "$probe Template"
      description = "Temporary v1.0.25.1 acceptance template"
...
function Assert-PayloadManifest {
  $manifestPath = Join-Path $PackageRoot "PAYLOAD_MANIFEST.json"
  if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) { throw "Payload manifest missing: $manifestPath" }
  ...
  foreach ($entry in @($manifest.files)) {
    ...
    if (-not (Test-Path -LiteralPath $full -PathType Leaf)) { throw "Manifest payload file missing: $rel" }
    $actualSize = (Get-Item -LiteralPath $full).Length
    if ($actualSize -ne $expectedSize) { throw "Payload size mismatch for ${rel}: expected $expectedSize, got $actualSize" }
    $actualHash = (Get-FileHash -LiteralPath $full -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($actualHash -ne $expectedHash) { throw "Payload SHA256 mismatch for $rel" }
  }
  Log "PASS payload manifest SHA256 and size verification."
}
...
try {
  ...
  Write-Host ""
  Write-Host "SUCCESS: v1.0.25.1 accepted." -ForegroundColor Green
  ...
  Write-Host ""
  Write-Host "v1.0.25.1 NOT ACCEPTED: $message" -ForegroundColor Red
